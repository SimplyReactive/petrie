...
<?php
    ...
    if ($isProfileUpdate) {
        $profileFormAction = route('profile.update');
        $passwordFormAction = route('profile.password');
    } else {
        $profileFormAction =  route('users.update', $user->hash);
        $passwordFormAction = route('password.change', $user->hash);
    }
?>
...
<div class="row">
    ...
    <div class="well">
        <form method="POST" action="{{ route('users.memberships', $user->hash) }}" accept-charset="UTF-8" class="form-horizontal" role="form">
        <form method="POST" action="{{ route('users.memberships', $user->hash) }}" accept-charset="UTF-8" class="form-horizontal" role="form">
